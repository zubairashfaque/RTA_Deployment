...
# Define the path to the compressed model file
...
def load_compressed_model(file_path):
    with bz2.BZ2File(file_path, 'rb') as f_in:
        # Load the compressed model using pickle
        loaded_model = pickle.load(f_in)
    return loaded_model

# Load the model
loaded_model = load_compressed_model(compressed_model_file)

# Load ordinal encoder mapping
loaded_encoder = joblib.load('data/processed/ordinal_encoder.pkl')

# Load target encoder mapping
loaded_target_encoder = joblib.load('data/processed/target_encoder.pkl')
...
def predict_severity(number_of_casualties, minute, Age_band_of_driver, number_of_vehicles_involved,
    ...
    # Map numerical prediction to severity label and symbol
    severity_label = severity_mapping.get(prediction[0], "Unknown")
    severity_symbol = severity_mapping_display.get(prediction[0], "❓")

    return severity_label, severity_symbol

# Add a button to trigger prediction
if st.sidebar.button('Predict Severity'):
    severity_label, severity_symbol = predict_severity(number_of_casualties, minute, Age_band_of_driver, number_of_vehicles_involved,
                                    Light_conditions, Day_of_week, Types_of_Junction, session, hour, Lanes_or_Medians)
